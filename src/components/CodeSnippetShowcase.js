...
import { Code2, Copy, Check, Terminal as TerminalIcon, Sparkles } from "lucide-react";
    ...
    <section id="craftsmanship" className="relative py-20 overflow-hidden">
      <div className="section-container relative z-10" ref={ref}>
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-mono font-semibold transition-all duration-300 cursor-pointer ${
                activeTab === key
                  ...
                  : "glass-card text-zinc-600 dark:text-zinc-400 border border-zinc-200/50 dark:border-white/10 hover:border-violet-500/30"
              }`}
            >
              {snippets[key].title}
            </button>
        ...
        <motion.div
          ...
          className="glass-card rounded-3xl border border-zinc-200/50 dark:border-white/10 bg-zinc-950 text-white shadow-2xl overflow-hidden max-w-4xl mx-auto font-mono"
        >
          {/* Editor Header Bar */}
          <div className="px-5 py-3.5 bg-zinc-900 border-b border-zinc-800 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] text-zinc-500">({currentSnippet.language})</span>
            </div>
            <button
              onClick={handleCopy}
              className="flex items-center gap-1.5 text-xs text-zinc-400 hover:text-white px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 transition-colors cursor-pointer"
            >
          ...
          {/* Description banner */}
          <div className="px-6 py-3 bg-zinc-900/50 border-b border-zinc-800/60 text-xs text-zinc-400 font-sans">
            💡 <span className="font-semibold text-zinc-300">Architecture Note:</span> {currentSnippet.description}
          </div>
